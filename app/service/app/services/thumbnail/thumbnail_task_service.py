#!/usr/bin/env python3
"""
Thumbnail Task Service for TissueLab AI Service
Provides asynchronous thumbnail generation using a background worker queue
"""
...
class ThumbnailWorker:
    """Asynchronous thumbnail worker backed by a background task queue"""
    ...
    def __init__(self, zmq_port: int = None, max_workers: int = None):
        ...
        self.thumbnail_generator = ThumbnailGenerator(max_workers=self.max_workers)
    ...
    def start_worker(self):
        """Start the thumbnail task worker thread"""
        if self.running:
            logger.warning("Thumbnail task worker is already running")
            return
            
        self.running = True
        self.worker_thread = threading.Thread(target=self._worker_loop, daemon=True)
        self.worker_thread.start()
        logger.info(f"Thumbnail task worker started on port {self.zmq_port}")
        
    def shutdown(self):
        """Shutdown the thumbnail task service"""
        if not self.running:
            return
        ...
        logger.info("Thumbnail task service shutdown complete")
