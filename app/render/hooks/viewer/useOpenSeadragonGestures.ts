...
export const useOpenSeadragonGestures = ({
  ...
  useEffect(() => {
    if (annotatorInstance && viewerRef.current) {
      ...
      const viewerElement = viewer?.element as (HTMLElement & { _wheelHandler?: EventListener }) | null;
      ...
      const existingWheelListeners = viewerElement.querySelectorAll('*');
      ...
      viewerElement._wheelHandler = wheelListener;
      viewerElement.addEventListener('wheel', wheelListener, { passive: false, capture: true });

      // Return cleanup function for all gesture handlers
      const cleanupGestures = () => {
        if (viewerElement) {
          viewerElement.removeEventListener('wheel', wheelListener, { capture: true });
          delete viewerElement._wheelHandler;
        }
      };
      ...
      if (viewer && viewer.container) {
        ...
        const viewportUpdateHandler = () => {
          ...
          // Get rotation angle in degrees (0, 30, 60, 90, ..., 360)
