...
type NodeLogsDialogProps = {
  ...
  env?: string
  port?: number
  node?: string  // Model name for API call
  pollMs?: number
}
...
export const NodeLogsDialog: React.FC<NodeLogsDialogProps> = ({
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      try {
        // API accepts model_name parameter
        const resp = await apiFetch(`${AI_SERVICE_API_ENDPOINT}/tasks/v1/logs/tail?model_name=${encodeURIComponent(node)}&n=200`, {
          method: 'GET',
          returnAxiosFormat: true,
        })
        const data = resp.data as { tail?: string; error?: string }
        const tail = data?.tail || ''
        const error = data?.error
        if (error) {
          setLogText(`[Remote Error] ${error}\n\n${tail}`)
        } else {
          setLogText(tail)
        }
      } catch (e) {
      ...
      <DialogContent className="w-[90vw] max-w-[700px] overflow-hidden gap-2">
        ...
        <div className="flex items-center justify-between text-xs text-slate-600">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" className="h-7" onClick={() => setPaused(p => !p)} disabled={!node}>
              {paused ? 'Resume' : 'Pause'}
            </Button>
        ...
        <div className="mt-2 border rounded overflow-hidden">
          ...
          <div className="bg-slate-950 h-80 overflow-y-auto overflow-x-hidden" ref={scrollRef}>
            <pre className="m-0 p-2 text-xs font-mono whitespace-pre-wrap break-words text-slate-100 leading-5 select-text w-full max-w-full">
              {logText || (node ? 'Tailing logs...' : 'No log file to display.')}
            </pre>
          </div>
        </div>
      </DialogContent>
