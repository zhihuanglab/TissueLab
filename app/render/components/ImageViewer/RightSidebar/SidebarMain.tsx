...
import { setTrackpadGesture } from '@/store/slices/viewer/viewerSettingsSlice';
import { setImageSetting, resetImageSettings } from '@/store/slices/viewer/imageSettingsSlice';
...
const getRandomColor = () => {
  // Generate random color and ensure it's not black (#000000)
  let color;
  do {
    ...
    color = '#' + randomNum.toString(16).padStart(6, '0').toUpperCase();
  } while (color === '#000000');
...
const MainSidebar: React.FC = () => {
  ...
  const handleSnapshot = async () => {
    if (!viewerInstance) {
      toast.warning('Viewer instance not available');
      return;
    }
  ...
  const handleColorChange = (index: number, color: string) => {
    let fixed = color.startsWith('#') ? color : `#${color}`;
    
    // If black color is selected, automatically generate a random non-black color
    if (fixed === '#000000') {
      // Generate a random number between 1 and 16777215 (0xFFFFFF) to avoid 0 (black)
      const randomNum = Math.floor(Math.random() * 16777214) + 1; // +1 to avoid 0
      fixed = '#' + randomNum.toString(16).padStart(6, '0').toUpperCase();
    }
          ...
          <Table className="compact-table">
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell className="py-1 px-2">
                  {slideInfo.mpp != null && slideInfo.mpp > 0
                    ? `${formatMppDisplay(Number(slideInfo.mpp))} µm`
                    : 'N/A'}
                </TableCell>
              </TableRow>
              <TableRow>
